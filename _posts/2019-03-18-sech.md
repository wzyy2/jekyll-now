...
* iotask的配置合理性通过qos判断.主要的指标:
    * 丢包
    * 延迟
* 对于重要的部分, 要通过配置满足在99.999%情况下丢不会出现task超过deadline, topicn不会出现延迟/丢包的情况

我觉得这种方式做到就很不错了, 像apollo那样的协程是没必要的.  

### 优化
apollo里没有提供给我们如何优化task调度的意见, 还是业务无关的角度上做的通用调度器.  
所以只能自己想想.... 

如何优化根据dag, event-based, time-based这些概念优化任务调度呢?.....  
构建一条高效的pipeline出来.  
