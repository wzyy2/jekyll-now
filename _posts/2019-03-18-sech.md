...
* 以task为单位提供执行timeline/占用率等perf能力
    * 以此衡量调度配置的合理性, 主要的指标:
        * 确定性!
        * deadline!
* iotask的配置合理性通过qos判断, 主要的指标:
    * 丢包
    * 延迟
* 对于重要的部分, 要通过配置满足在99.999%情况下不会出现task超过deadline, io不会出现延迟/丢包.

我觉得这种方式做到就很不错了, 像apollo那样的协程是没必要的.  

### 3.2.1. 优化
apollo里没有提供给我们如何优化task调度的意见, 还是业务无关的通用调度器.  
所以只能自己想, 如何根据dag, event-based, time-based这些概念优化任务调度呢? 构建一条高效的pipeline出来.  

不过可能也没必要, 因为**延迟**最小并不是一个很关键的需求.  

在task/io的角度上做到**确定性**, 从而整体系统形式上的**延迟**可控, 可计算, 才是构建可靠系统的关键.  
**系统性的大问题分解到小问题, 从链路延迟到任务延迟.**
