...
graphic，提供opengles api，适合做3d图形和2d图形的加速。

# software

[](https://upload.wikimedia.org/wikipedia/commons/f/ff/Linux_AMD_graphics_stack.svg)
先看这个结构图，这些图看懂了就差不多了，有什么不理解可以留言
重点理解libdrm，wayland和x11(compositor),mesa和libmali，qt和gtk（applcation）的关系。

之前在github解释了一下rockchip平台的支持现状，可以看看
https://github.com/rockchip-linux/rk-rootfs-build/issues/25

## kernel

[交换图层上下关系](https://github.com/rockchip-linux/meta-rockchip-extra/blob/master/demo/meta-player-qt/linux-rockchip/linux-0007-drm-rockchip-make-video-overlay-bottom-layer.patch)

plane id和crtc idd的获取办法：我一般在内核打印。。。。。
...
* libdrm的演进比较快，比如说api还分atomic和legacy，你可能看到的资料和你现在用的完全不一样，只是有熟悉演进记录的人才行。
* 很多效果依赖于厂商的实现，甚至rockchip还有修改很多core API的表现效果还辅助我们内部产品项目，所以同个API在不同平台表现有可能完全不同的

所以，如果广告机循环播放视频，那最好是用gstreamer，然后选kmssink显示，而不是直接调用libdrm的api。
如果还是打算直接用的话，那么下面是一些资料。

### 简单的理解

drm里有crtc，plane这两个东西，可以理解
* crtc就是屏幕，比如一个hdmi一个crtc num， 一个dsi一个crtc num
...
就和一般平台差不多，不过要x11的性能缺陷问题，见最下面的rockchip现状。
...
### gstreamer
提供rkximagesink和kmssink。

### mpv
upstream mpv目前用的是opengles的videooutput

### rockchip现状

Let me explain rockchip graphic current status, hope it could help people who need those infomations.

At the begining, we want to use 2D accel unit in xserver, just like other platforms, but our 2D accel unit don't have a mmu, which means it's not suitable for EXA, so we switch to glamor.  
The problem about glamor is that libmali is not mesa, they behave different in many places, which make it so buggy. The results show it can only accel big operations(drag window) and even make small operations(gtk) much slower.  
Optimizating xserver is beyond our ability and we have been told that ARM won't support X11 in the future, so we won't have dev plan for it.

We have plan to switch to wayland(gnome, kde etc) for people who want a desktop enviroment, but not have test it.
