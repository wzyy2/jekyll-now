...
如何基于决策和路径规划的输出来做Vote的实现, 没想明白.....是个值得研究的问题. 或者就只是简单的根据监控的结果, 对失效平台传过来的数据做丢弃.

备份安全平台:

备份安全平台是我们容错设计的核心部分

关于熔断安全, 可以参考如下waymo的描述.

    含碰撞检测及碰撞规避系统在内的多个备用系统可持续扫描车辆前后的物体，包括：行人、骑行者及其它车辆。若主系统未探查到行驶路径中的目标物（极少数情况），这类备用系统可控制车辆，逐步减速直至安全停靠。

鉴于备份安全平台的计算能力, 数据来源一般都是用2D激光/超声波这种false/true数据, 或者毫米波雷达/MobileEye这种直接输出tracklet数据, 所以效果必然是比主系统差的. 主要覆盖的是万一情况, 比如说主系统更新了bug版本 : )
