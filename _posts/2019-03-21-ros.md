...
![](http://blog.iotwrt.com/images/rosi.png)
...
ros::spin()；
```
在`timer_func`外的空闲时间, 程序会处理`topic callback`, 从而减少丢包和Proc的延迟波动
...
对接受者来说, 会浪费执行时间在`pollmanager/spinner` 高频topic处理上.
