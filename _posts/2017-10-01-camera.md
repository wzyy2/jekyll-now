...
这套驱动把很多原来被Wrap住的细节暴露给了用户层，这样一来，应用层怎么使用就是个问题。

我们最先开发的是Android CamHal3。对CamHal3来说，这种按照规范暴露出的细节其实很方便开发者（看看我们CamHal3到处都是intel的header就知道了，逃。。。）。
对使用者来说，使用方式也不会有差异，细节都在xml里被vendor提前隐藏了。

然后回到Linux，我们用的是[rkcamsrc](https://github.com/rockchip-linux/gstreamer-rockchip-extra)（类似的nvidia有nvcamerasrc，intel有xcamsrc）。一开始我是打算写成类似CamHal的做法----就是使用者不用关心硬件细节，不用操作media controller，不用手动设置各个模块的format----只需要通过下request，确定拍照还是录像还是预览，分辨率要多大---但后面发现这样的话，如果要做的完整，不损失功能，需要的工作量太大了。另外有很多奇奇怪怪的sensor，比如说HDMI IN，这么写的话，要怎么满足他的需求？

Rockchip也只是Chip Vendor，不是Google，抽象这些太麻烦了。其实参考TI和nvidia的做法，他们也是暴露了media controller给用户。
所以在Linux使用的话，还是得User自己去接触实际硬件和基础软件，来满足自己想要的需求。
...
和预览没有区别，gstreamer上source接mpph264enc即可。
