...
SDK通过github对外发布，基础的Linux_SDK下载命令如下，包含了Kernel，U-Boot，Debian操作系统，相关工具等：
repo init --repo-url=https://github.com/rockchip-linux/repo -u https://github.com/rockchip-linux/manifests -b master
repo sync

下载的代码，Kernel使用release-4.4分支，U-Boot使用release分支，代码皆为滚动更新，如有需要可以切换至tag标记的分支。
基础组件代码，比如gstreamer-rockchip，mpp并没有同步，如有需要请上github获取。其版本和所用系统绑定，debian系统请下载debian分支的代码。
...
自己下载编译代码，请一定要注意版本（可以在commit log或者文件名里看）。我们现在github的代码推送是滚动的，但是我们放在yocto和debian里相关组件的版本都是固定的，随便切取版本不一定能使用。不编译代码也可以使用的，只要是我们提供的debian rootfs和yocto。
...
mpv和ffmpeg的代码和编译选项看bitbake文件：<br>
https://github.com/rockchip-linux/meta-rockchip/tree/master/recipes-multimedia

比较麻烦，建议使用yocto，选中这两个package就可以了。
```
...
测试办法：
```
mpv --hwdec=rkmpp --vo=drm test.mp4
```
